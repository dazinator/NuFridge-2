        ...
        this.CanGoToNextStep = ko.computed(function () {
            ...
            if (anotherStepAvailable) {
                if (self.ActiveStepIndex() == 2) {
                    var databaseSystemIsValid = self.Install().DatabaseSystem.isValid();
                    if (!databaseSystemIsValid) {
                        return false;
                    }
                    var databaseSystem = self.Install().DatabaseSystem();
                    if (databaseSystem == 'Mongo') {
